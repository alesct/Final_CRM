...
with 탭2:
    ...
    if 원본df is None:
        ...
        월컬럼 = next((c for c in df.columns if "month_num" in c.lower()), None)
        ...
        if not 카테고리컬럼 or not 매출컬럼:
            ...
            if 고객컬럼 and bikes_mask.sum() > 0:
                bikes_고객 = set(
                    pd.to_numeric(국가필터df.loc[bikes_mask, 고객컬럼], errors="coerce")
                    .dropna().astype(int).unique()
                )
                bikes_고객 = {k for k in bikes_고객 if k > 0}

            if 고객컬럼 and len(bikes_고객) > 0:
                동반구매_df = 국가필터df[
                    pd.to_numeric(국가필터df[고객컬럼], errors="coerce").isin(bikes_고객) &
                    (~bikes_mask)
                ]
            else:
                동반구매_df = 국가필터df[~bikes_mask]

            동반df_raw = (동반구매_df.groupby(카테고리컬럼)[매출컬럼]
            ...
            with k1:
                st.markdown(f'<div class="kpi-card" style="border-top-color:#7b93a8;"><div class="kpi-val">${bikes_총매출/1_000_000:.2f}M</div><div class="kpi-label">{t("Bikes 총 매출")}</div></div>', unsafe_allow_html=True)
            with k2:
            ...
            if 월컬럼:
                ...
                if len(유효df) > 0:
                    계절카테고리 = (유효df.groupby(["_계절", 카테고리컬럼])[매출컬럼]
                        .sum().reset_index())
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.markdown(f'<div class="section-header"><div class="section-dot" style="background:#c4956a"></div><span>{t("동반 구매 카테고리별 매출")}</span></div>', unsafe_allow_html=True)
                if len(동반df_raw) > 0:
                    fig3 = go.Figure(go.Bar(
                        ...
                        textfont=dict(size=13, color="#8c8480"),
                        customdata=동반df_raw["구매건수"],
                        hovertemplate="%{x}<br>$%{y:,.0f}<br>%{customdata}건<extra></extra>"
                    ))
                    pastel_layout(fig3, height=280, margin=dict(l=10, r=20, t=20, b=10))
                    fig3.update_yaxes(title_text=t("매출 ($)"), tickformat="$,.0f")
                    st.plotly_chart(fig3, use_container_width=True)

                st.markdown(f'<div class="section-header"><div class="section-dot" style="background:#7b93a8"></div><span>{t("시즌별 카테고리 매출 패턴")}</span></div>', unsafe_allow_html=True)
                if len(계절카테고리) > 0:
                    fig4 = go.Figure()
                    for 카t in 계절카테고리["카테고리"].unique():
                        sub = 계절카테고리[계절카테고리["카테고리"] == 카t]
                        sub = sub.set_index("계절").reindex(계절순서).reset_index().fillna(0)
                        계절_표시 = [t(s) for s in sub["계절"]]
                        fig4.add_trace(go.Bar(
                            name=카t, x=계절_표시, y=sub["매출"],
                            marker_color=카테고리색.get(카t, "#b8b0a8"), marker_line_width=0
                        ))
            ...
            with c2:
                ...
                전략맵_ko = {
                    "Accessories": lambda 행, 국가: f"{국가} 자전거 구매 고객 {int(행['구매건수']):,}명이 Accessories를 동반 구매했습니다 (${행['총매출']/1000:.0f}K). 자전거 판매 시점에 헬멧·타이어·잠금장치 번들을 함께 제안하세요.",
                    "Components": lambda 행, 국가: f"{국가} 자전거 구매 고객 {int(행['구매건수']):,}명이 Components를 동반 구매했습니다 (${행['총매출']/1000:.0f}K). 고관여 고객에게 프레임·휠 업그레이드 패키지를 추가 제안하세요.",
                    "Clothing": lambda 행, 국가: f"{국가} 자전거 구매 고객 {int(행['구매건수']):,}명이 Clothing을 동반 구매했습니다 (${행['총매출']/1000:.0f}K). 결제 완료 화면에서 저지·반바지 세트를 즉시 추천하세요.",
                }

                if len(동반df_raw) > 0:
                    for _, 행 in 동반df_raw.iterrows():
                        cat = 행[카테고리컬럼]
                        색 = 카테고리색.get(cat, "#7b93a8")
                        top_subs = ""
                        if 서브카테고리컬럼 and len(동반구매_df) > 0:
                            cat_rows = 동반구매_df[동반구매_df[카테고리컬럼] == cat]
                            if len(cat_rows) > 0:
                                top_subs = ", ".join(
                                    cat_rows.groupby(서브카테고리컬럼)[매출컬럼]
                                    .sum().sort_values(ascending=False).head(3).index.tolist()
                                )
                        ko_text = 전략맵_ko[cat](행, 국가명표시) if cat in 전략맵_ko else f"동반 구매 ${행['총매출']/1000:.0f}K. 번들 프로모션을 실행하세요."
                        if top_subs:
                            ko_text += f" 주요: {top_subs}."
                        전략 = t(ko_text)
                        sub_html = f'<div style="margin-top:6px;font-size:12px;color:#7b93a8;">{t("주요 서브카테고리")}: {top_subs}</div>' if top_subs else ""
                    ...
                    st.info("동반 구매 데이터가 없습니다.")

    st.markdown("<hr style='border-color:#ddd8d0;margin:32px 0 24px 0;'>", unsafe_allow_html=True)
    ...
    with u3:
        ...
        up_월 = 계절_대표월[업셀_계절_키]
        st.caption(f"→ 월 {up_월} ({업셀_계절_표시})")
    with u4:
        up_국가_선택 = st.selectbox(t("국가"), [t("전체 국가")] + 국가목록, key="up_country")
    ...
    try:
        ...
        api_계절 = up_result.get("계절", 업셀_계절_키)
        api_ok = True
    except:
        카테고리별예측 = {
